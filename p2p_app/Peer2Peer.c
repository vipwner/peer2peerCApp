...
int open_socket()
{
    int sock = socket(PF_INET, SOCK_STREAM,
                      0); // El 0 indica que el SO escoge el protocolo.
    if(sock == -1) {
...
void server_process(int PORT)
{
    ...
    int listener =
        open_socket(); // Socket que escucha los clientes que se conectan.
    ...
    while(1) {
        ...
        int connect = accept(listener, (struct sockaddr *) &socket_client,
                             &address_size);
...
void client_process(int PORT)
{
    ...
    if(validateGET(message_clientProcess)) {
        ...
        for(it; it < loadT_ok; it++) {
            ...
            torrent.chunks[it].status =
                1; // 1 Indica que aun no se ha descargado, 0 lo contrario.
        }
        ...
        for(it; it < loadN_ok; it++) {
            ...
            printf("%d. PRUEBA DATOS nodos: %s, %s\n", it, nodesContainer[it].ipNode,
                   nodesContainer[it].portNode);
        }
        it = 0;
        ...
        for(it; it < nodeCount; it++) {
            ...
            server.sin_addr.s_addr = inet_addr(
                                         nodesContainer[it].ipNode); // Aquí se pone la IP del par con el cual se desea comunicar.
            server.sin_family = AF_INET;
            server.sin_port = htons(atoi(
                                        nodesContainer[it].portNode)); // El puerto del par con el que se desea comunicar.
            ...
            if(validateHAS(comand_HAS_Not)) {
                ...
                nodesContainer[it].chunks = (Chunk *) calloc (MAX_CONTAINER_NUM + it + 1,
                                            sizeof(Chunk));
                int setCNode_ok = setChunksNode(chunkList, chunkListLen,
                                                nodesContainer[it].chunks);
                if(setCNode_ok == -1) {
                ...
                for(it_in; it_in < setCNode_ok; it_in++) {
                    ...
                    printf("%d. PRUEBA DATOS nodes.chunks: %s\n", it_in,
                           nodesContainer[it].chunks[it_in].SHA);
                }
            }
        ...
        for(it; it < torrent.chunksQ; it++) {
            ...
            if(pid) {
                ...
                char rarestSHA[40];
                int rarest_ok = getRarestChunkFirst(rarestSHA, torrent.chunks, torrent.chunksQ,
                                                    nodesContainer, nodeCount);
                if(rarest_ok == -1) {
                ...
                int rarestNode_ok = searchNode(temp_id, temp_ip, temp_port, nodesContainer,
                                               nodeCount, rarestSHA);
                if(rarestNode_ok == -1) {
                ...
                struct sockaddr_in server;
                server.sin_addr.s_addr = inet_addr(
                                             nodeToCon.ipNode); // Aquí se pone la IP del par con el cual se desea comunicar.
                server.sin_family = AF_INET;
                server.sin_port = htons(atoi(
                                            nodeToCon.portNode)); // El puerto del par con el que se desea comunicar.
                // Iniciamos la conexion al servidor.
                int connect_TORARESTNODE = connect(listener, (struct sockaddr *)&server,
                                                   sizeof(server));
                ...
                char line[100];
                sprintf(line,"%s\t%s",torrent.chunks[rarest_ok].idChunk,
                        torrent.chunks[rarest_ok].SHA);
        ...
        server.sin_addr.s_addr = htonl(
                                     INADDR_ANY); // Aquí se pone la IP del par con el cual se desea comunicar.
        server.sin_family = AF_INET;
        server.sin_port = htons(
                              PORT); // El puerto del par con el que se desea comunicar.
        // Iniciamos la conexion al servidor.
        int connect_ = connect(listener, (struct sockaddr *)&server, sizeof(server));
        if(connect_ < 0) {
            perror("Imposible realizar conexion. Error\n");
